...
def add_rolling_features(df: pd.DataFrame):
    ...
    if "next_opponent_difficulty" in df.columns:
        df["next_opponent_difficulty"] = (
            df["next_opponent_difficulty"].combine_first(computed_next_opponent)
        )
    else:
        df["next_opponent_difficulty"] = computed_next_opponent

    if "next_team_difficulty" in df.columns:
        df["next_team_difficulty"] = (
            df["next_team_difficulty"].combine_first(computed_next_team)
        )
    else:
        df["next_team_difficulty"] = computed_next_team
